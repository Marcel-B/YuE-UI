...
- `WorkerHost` – singleton and hosted service. Starts the worker lazily on the first command, turns events into `SongState`s and a log, and fans changes out to subscribers (bounded channels; a slow subscriber is dropped and its EventSource reconnects with a fresh snapshot). Progress events are throttled per song. Each `SongState` keeps its stages with their start time (`Stages`, via `SongState.Entering`) and whether it is a render (`Render`, remembered from the render command since `started` does not say), which `SongTimeline.vue` shows with how long each stage took. If the process exits, running songs are marked failed and the next command starts a new worker.
...
The app's own state lives in one SQLite file (`DataOptions`, section `Data`, default `yueui.db` next to `push.json`), opened by `SqliteDatabase`, which creates the file on first use and versions the schema with `PRAGMA user_version` (add a step, never edit an old one; same pattern as yue-to-logic-pro). It holds the playlist (`SqlitePlaylistStore`): one row in `playlists`, its song ids in `playlist_songs`; and titles given to runs (`SqliteRunTitleStore`, `run_titles`). A rename (`PUT /api/runs/{run}/title`) never touches the folder, since its name is the song id that playlists, links, the worker's states and YuE Studio rely on; `SongLibrary` lays the title over the worker's (`RunInfo.OriginalTitle` keeps that one) and `WorkerHost.RunRenamed` renames the queue's songs. `SongLibrary` still lists songs when the database cannot be read. `PUT /api/playlist` replaces the whole list and only accepts ids of existing songs; `GET` leaves out songs deleted since. On the server rather than in `localStorage`, because the phone's home-screen app, its Safari and the Mac each have their own storage.
...
Vue 3 + TypeScript + Vite, no router, no state library. `App.vue` holds the state and the event subscription and switches pages (`view.ts`: create, transcribe, songs, playlist, kept in the URL hash, shown with `v-show` so a page keeps its input; a song's address is `#/songs/<run>/songN`, which scrolls the songs page to it and marks it, and player, playlist and queue link there) through a PrimeVue `Menubar`; `player.ts` owns the one audio element (in `PlayerBar.vue`, outside the pages, so playback survives a page change) and what plays next; `playlist.ts` holds the playlist's ids; `api.ts` is the only place that talks to `/api`; `types.ts` mirrors the API's records by hand; `i18n.ts` holds every German and English string; `form.ts` maps the form to `GenerateRequest` and keeps it in `localStorage`, and a song's `request.json` (`GET /api/songs/{run}/{song}/request`, `SongRequest`) back into the form for "as a new song" (`SongMenu.vue`). `styleTags.ts` holds the style building blocks (`StyleBlocks.vue`), picked from YuE v1's `top_200_tags.json` plus language and tempo, and inserts or removes them in the comma-separated style. `search.ts` filters the library in the browser (the listing already carries style and lyrics), in title and style or in the lyrics, never both, since common lyric words would bury the title hits. Mobile first (the main client is a phone): inputs are 16px to avoid iOS zoom, the player uses `preload="none"` and starts `play()` inside the click, which iOS requires.
